...
class FReview {
    ...
    private static $key = "reviewId";

    private static $class = "FReview";
    
    private static $table = "review";
    
    private static $values = "(NULL,:title,:rating,:description,:date,:userId)";
    ...
    public static function bind($stmt, $review) {
        ...
        $stmt->bindValue(":userId", $review->getUserId(), PDO::PARAM_INT);      
    }
    ...
    public static function createReview($queryRes){
        $review = new EReview($queryRes["title"], $queryRes["rating"], $queryRes["description"], $queryRes["date"], $queryRes["userId"]);
        if (isset($queryRes["reviewId"])) {
            $review->setId($queryRes["reviewId"]);
        }
        return $review;
    }
