...
class FUtente{
    ...
    private static $values = "(NULL,:nome,:cognome,:dataNascita,:comuneNascita)";
    ...
    public static function bind($stmt,$utente) { 
        ...
        $stmt->bindValue(":dataNascita", $utente->getDataNascita()->format("Y-m-d"), PDO::PARAM_STR);
    ...
    public static function creaOggetto($queryRes){
        $utente = new EUtente(null, $queryRes["nome"], $queryRes["cognome"], $queryRes["dataNascita"], $queryRes["comuneNascita"]);
        if (isset($queryRes["idUtente"])) { 
            $utente->setId($queryRes["idUtente"]);
        }
        return $utente;
    }
