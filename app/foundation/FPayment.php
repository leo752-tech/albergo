...
class FPayment{
    ...
    private static $values = "(NULL,:idBooking,:amount,:paymentDate,:lastFourDigits,:cardHolderName)";
    ...
    public static function bind($stmt,$payment) {
        ...
        $stmt->bindValue(":cardHolderName", $payment->getCardHolderName(), PDO::PARAM_STR);
    }
    ...
    public static function createObject($queryRes){
        $payment = new EPayment($queryRes["idPayment"], $queryRes["idBooking"], $queryRes["amount"], new DateTime($queryRes["paymentDate"]), $queryRes["lastFourDigits"], $queryRes["cardHolderName"]);
        return $payment;
    }
    ...
    public static function getAll(){
        $payment = FDataMapper::getInstance()->selectAll(self::$table);
        return $payment;
    }
}
