...
class FImage{
    ...
    private static $class = "FImageSpecialOffer";
    
    private static $table = "image_offer";
    
    private static $values = "(NULL,:idSpecialOffer,:name,:filePath,:mimeType)";
    
    public function __construct(){}

    public static function bind($stmt,$image) {
        $stmt->bindValue(":idSpecialOffer", $image->getIdSpecialOffer(), PDO::PARAM_INT);
    ...
    public static function createObject($queryRes){
        $image = new EImage($queryRes["idImage"], $queryRes["idSpecialOffer"], $queryRes["name"], $queryRes["filePath"], $queryRes["mimeType"]);
        return $image;
    }
    ...
    public static function getImagesByOffer($idSpecialOffer){
        try{
            $images = self::getAll();
            $imagesRo = array();
